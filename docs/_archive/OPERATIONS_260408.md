...
| 항목 | 값 |
|------|----|
| 매일 1줄 명령 | `source .venv/bin/activate && python run_integrated_analysis.py` |
| 레거시 파이프라인 | `python run_full_pipeline.py` (AI 분석 + GBM 포함 9단계) |
| 일일 스케줄러 | `python run_daily_scheduler.py` (1회) / `--status` (상태) / `--install-cron` (자동화) |
| 대시보드 | `cd dashboard && python3 -m http.server 8889` → http://localhost:8889 |
...
result/             ← 날짜별 스크리닝 (smart_money_picks_YYYYMMDD.csv)
reports/            ← 일일 종합 리포트 (daily_report_YYYYMMDD.json + latest_report.json)
logs/               ← 실행 로그 (daily_run_YYYYMMDD.log)
ml/                 ← GBM 피처/모델/검증 (Part 5)
.docs/evolution/    ← memory.md + backlog.md + 사이클 로그
dashboard/          ← 웹 대시보드 (정적 파일)
```
...
- [ ] **통합 분석 실행**: `python run_integrated_analysis.py` (또는 cron 자동 실행 확인)
- [ ] **Verdict 확인**: `reports/latest_report.json` 의 `verdict` 필드 (GO / CAUTION / STOP)
- [ ] **체제 변경 여부**: `output/regime_result.json` 의 `regime` 필드가 전일과 다른지
  - `neutral → risk_off` 같은 전환은 주목 (적응형 파라미터 바뀜)
- [ ] **Action 분포**: `reports/latest_report.json` 의 `summary.action_distribution` — BUY 종목 수 확인
...
- [ ] **스케줄러 상태** (cron 설정 시): `python run_daily_scheduler.py --status`
...
- [ ] `reports/daily_report_YYYYMMDD.json` 파일 생성 확인 (오늘 날짜)
- [ ] `result/smart_money_picks_YYYYMMDD.csv` 파일 생성 확인 (오늘 날짜)
...
| 주기 | 작업 | 근거 / 명령어 |
|------|------|---------------|
| **주 1회 (월)** | IndexPredictor 수동 재학습 | `python us_market/index_predictor.py` (7일 경과 시 자동이지만 수동 권장) |
| **주 1회** | `backlog.md` P0 리뷰 + 다음 사이클 선정 | service-evolver Phase 4 |
| **주 1회** | API 비용 누적 점검 (Gemini/GPT/Perplexity) | 콘솔 로그 집계 |
| **월 1회** | 주가 데이터 full refresh | `python collectors/fetch_sp500_prices.py` (503종목 × 1년) |
| **월 1회** | walk-forward 재검증 (PBO/DSR) | `python -m ml.validation.walk_forward` |
...
| 디스크 용량 | `du -sh output/ result/ ml/` | 각 <500MB | 오래된 `predictor_model_*.joblib` 정리 |
...
| 파일 누적 | `ls result/*.csv \| wc -l` | 월별 ~22개 | 없으면 파이프라인 실패 이력 |

**월 1회**: `python pipeline/data_quality_report.py` 로 CSV 100점 채점.
...
| **Performance drift** (정확도 하락) | `model_accuracy` 5일 이동평균 | <0.50 **5일 지속** | 수동 재학습 `python us_market/index_predictor.py` |
...
| `ValueError: format code 'f' for str` | `confidence` (str) vs `confidence_pct` (float) 혼동 | `.docs/evolution/memory.md` Don'ts #1 참조 |
...
| `run_full_pipeline.py Step 7 FAIL` | LightGBM/pyarrow 미설치 | `pip install lightgbm pyarrow` + macOS는 `brew install libomp` |
| `Step 8 IndexPredictor FAIL` | 2년치 SPY 데이터 부족 | `yfinance.download('SPY', period='2y')` 수동 테스트 |
| `final_report_generator 빈 결과` | smart_money_picks CSV 없음 | Step 4 `run_screening.py` 먼저 실행 |
...
4. **실패 로그 저장**: 새 이슈는 `.docs/evolution/memory.md` Don'ts 에 append
...
### 백업 정책
- **git**: 코드 + `.docs/evolution/*` 은 필수. `output/final_top10_report.json` 은 선택.
- **모델**: `output/predictor_model_*.joblib`, `ml/models/*.pkl` **백업 불필요** (재학습 가능).
...
```bash
# CSV 전체 재생성 (~20분)
python pipeline/run_pipeline.py --top-n 503 --period 1y --output-dir data
...
### 단계 전환 gate
- `python -m ml.validation.walk_forward` → `VERDICT=PASS`
...
| "어제 뭐 바꿨지?" | [.docs/evolution/memory.md](../.docs/evolution/memory.md) + 최근 cycle log |
| "할 일은 뭐?" | [.docs/evolution/backlog.md](../.docs/evolution/backlog.md) |
...
**마지막 업데이트**: 2026-04-08 (통합 파이프라인 + 스케줄러 + Strategy/Setup + Verdict/Action 반영)
**다음 리뷰**: 2026-05-08 (월간 캘리브레이션)
