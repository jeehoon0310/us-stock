...
> **버전**: v3 (2026-04-13)
> **소스**: 2026-04-12 debug 세션 (10+ 실패 run → 최종 성공) + 2026-04-13 SCP chmod 수정
> **다이어그램**: [ci-flow.drawio](./ci-flow.drawio)
> **이전 버전**: v2 (2026-04-12, `_archive/ci-flow_v2_260412.md`) — 초기 배포 성공 후 기록
...
## 13. 2026-04-13 변경 이력 (v2 → v3)

### 13.1 SCP chmod 에러 수정 (PR #22 관련)

**증상**: deploy.yml 배포 스크립트 실행 중 `tar: cannot change ownership` 에러 발생

**원인**: `/volume1/docker/us-stock` 디렉토리가 없거나 권한이 `jeehoon` 사용자에 없음

**수정 내용** (deploy.yml ssh-action script 앞부분):
```bash
# 추가된 2줄
sudo mkdir -p /volume1/docker/us-stock
...
### 13.2 nginx 라우팅 레이어 추가 (edu.frindle.synology.me/notice 404 수정)

**증상**: `https://edu.frindle.synology.me/notice` 접속 시 404 반환

**원인**: DSM Reverse Proxy가 Next.js(:8889)를 직접 가리키는데, Next.js App Router에 `/notice` 라우트가 없음

**해결책**:
1. `dashboard/nginx.conf`에 `proxy_pass http://192.168.1.11:8889` 추가 (Next.js 전달)
2. DSM Reverse Proxy 대상 변경: `:8889` → `:8080` (nginx)

**결과 라우팅**:
```
edu.frindle.synology.me → DSM :8080 (nginx)
  ├─ /notice, /pages/*.html → nginx 정적 서빙
  └─ /regime, /top-picks, / → proxy_pass :8889 (Next.js)
```

### 13.3 notice.html 콘텐츠 업데이트

- 쌍둥이 아빠 텍스트 3곳 삭제
- 신청하기 버튼에 Tally.so 링크 연결: `https://tally.so/embed/mOx9Wp?hideTitle=1&utm_source=navercafe`
...
| `tar: cannot change ownership` 또는 SCP chmod 에러 | SSH script log | `/volume1/docker/us-stock` 디렉토리 권한 문제 | `sudo mkdir -p` + `sudo chown -R jeehoon /volume1/docker/us-stock` 추가 후 재실행 |
| `edu.frindle.synology.me/notice` 404 | 브라우저 | DSM Reverse Proxy가 Next.js(:8889)를 직접 가리킴 — Next.js에 /notice 라우트 없음 | DSM 프록시 대상을 `localhost:8080` (nginx)으로 변경 |
