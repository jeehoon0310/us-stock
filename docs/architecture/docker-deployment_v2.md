...
Synology Container Manager
  ...
  us-stock-dashboard 컨테이너 (nginx, bridge network, :8080)
    ...
    ├─ /images/ → 정적 파일 직접 서빙
    └─ / (나머지) → proxy_pass http://192.168.1.11:8889 (Next.js)
                ↓
...
- `dashboard/index.html` → `dashboard/pages/terminal.html` 이동 완료 (Yahoo Finance 퍼플 스타일, NOTICE 모달)
- `dashboard/daily_report.html` → `dashboard/pages/reports.html` 이동 완료
- `dashboard/pages/notice.html` 신규 생성 (강의 공지)
- `dashboard/images/` 신규 생성 (이미지 에셋 전용)
- `dashboard/_ref/` 참고용 시안만 보관 (수정 금지)
